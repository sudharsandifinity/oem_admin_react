...
const Itemtable = (props) => {
  ...
  const handlechangeUser = (e) => {
    const selectedUserId = Number(e.target.value || e.detail?.selectedOption?.value);

    const selectedUser = userList.find(
      (user) => user.InternalKey === selectedUserId,
    );
    ...
    setSummaryData((prev) => ({
      ...prev,
      InternalKey: selectedUser?.InternalKey ?? "",
      RequestorName: selectedUser?.UserName ?? "",
  ...
  const columns = useMemo(() => {
    ...
    const allColumns = [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <Input
            value={row.original.fulldescription}
            readonly
            disabled={mode === "view"}
            style={{
              border: "none",
            ...
            onBlur={(e) => (e.target.style.borderBottom = "1px solid #ccc")}
            onClick={() =>
              !row.original.fulldescription &&
              openitemDialog(row.index, "fulldescription")
            }
          />
        ),
      },
      ...
      ...(location?.state?.formname === "Material Request" ||
        ...
        ? [
            {
              ...
              Cell: ({ row }) => (
                <>
                  <Input
                    value={
                      row.original.bomopenqty ? row.original.bomopenqty : ""
                    }
                    readonly
                    disabled={mode === "view"}
                    style={{ textAlign: "right" }}
...

      {
        ...
        Cell: ({ row }) => (
          <>
            <Input
              value={row.original.issuedQty ? row.original.issuedQty : ""}
              readonly
              disabled={mode === "view"}
              style={{ textAlign: "right" }}
              onFocus={(e) =>
...

      {
        ...
        Cell: ({ row }) => (
          <>
            <Input
              value={row.original.inStock ? row.original.inStock : ""}
              readonly
              disabled={mode === "view"}
              style={{ textAlign: "right" }}
              onFocus={(e) =>
    ...
    const visibleColumns = allColumns.filter(
      (col) =>
        visibleAccessors.includes(col?.accessor) || col?.id === "actions", // always include actions
    );
            ...
            <FlexBox alignItems="Center" style={{ gap: "1rem" }}>
              ...
              >
                ...
                {summaryData?.RequestorCode === "1"
                  ? userList.length > 0 &&
                    userList.map((user) => (
                      <Option key={user.InternalKey} value={user.InternalKey}>
                        {user.UserName}
                      </Option>
                    ))
              ...
              <Input
                ...
                value={
                  summaryData?.Department
                    ? departmentList?.find(
                        (dept) => dept.Code === Number(summaryData?.Department),
                      )?.Name || ""

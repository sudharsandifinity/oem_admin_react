...
import { HeaderFilterBar } from "../ManageSalesOrder/HeaderFilterBar";
import ViewSalesOrder from "../ManageSalesOrder/ViewSalesOrder";
...
const Inventory = () => {
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      try {
        let res = "";
        if (formDetails[0]?.name === "Inventory ") {
          res = await dispatch(
                  ...
                  <div>
                    <FlexBox direction="Column">
                      <AnalyticalTable
                        columns={columns}
                        data={tableData}
                        // header={`(Inventory  - ${tableData.length})`}
                        rowStyle={(row) => {
                          const docLines = row.original?.DocumentLines || [];
                          const allQtyZero =
                            Array.isArray(docLines) &&
                ...
                <Page
                  header={
                    <Bar
                      ...
                      startContent={
                        <Title level="H5">Preview Inventory </Title>
                      }
                    ></Bar>
                  }
                >

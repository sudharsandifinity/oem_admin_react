...
const Form = ({
  ...
  const handleAddRow = () => {
    if (!newRow.name || !newRow.display_name)
      return alert("Please fill all fields");

    setFormTabs([...formTabs, newRow]);
    setNewRow({ name: "", display_name: "", status: 1 });
  };

  const handleEditRow = (rowIndex) => {
    setEditIndex(rowIndex);
    setNewRow(formTabs[rowIndex]);
  };

  const handleUpdateRow = () => {
    const updated = [...formTabs];
            ...
            >
              ...
              <FlexBox style={{ gap: "1rem" }}>
                ...
                <Input
                  ...
                  onInput={(e) =>
                    setNewRow({ ...newRow, display_name: e.target.value })
                  }
                />
                  {console.log("newRow",newRow)}
                <Select
                  value={newRow.status===1?"Active":"Inactive"}
                  onChange={(e) =>
                    setNewRow({
                      ...newRow,

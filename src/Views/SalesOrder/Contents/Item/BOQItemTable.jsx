...
const BOQItemTable = (props) => {
  ...
  const columns = useMemo(() => {
    ...
    const allColumns = [
      ...
      {
        Header: "Country Region of Origin",
        accessor: "region",
        Cell: ({ row }) => (
          <Input
      ...
      },
      {
        ...
        Footer: () => (
          <div style={{ textAlign: "center", fontWeight: "bold" }}>
            {"Total: " + summaryCalculation.totalBeforeDiscount}
          </div>
        ),
      },
      {
  ...
  const totals = useMemo(() => {
    return itemTabledata.reduce(
      (acc, row) => {
        acc.quantity += Number(row.quantity || 0);
        acc.amount += Number(row.amount || 0);
        acc.discount += Number(row.discount || 0);
        acc.baseAmount += Number(row.BaseAmount || 0);
        acc.tax += Number(row.TaxRate || 0);
        acc.gross += Number(row.grosstotal || 0);
        return acc;
      },
      {
        quantity: 0,
  ...
  return (
    <div style={{ padding: "1rem" }}>
      ...
      >
        ...
        {/* Unit Price */}
        <div style={{ width: 120, textAlign: "right", padding: "8px" }}>
        ...
        {/* Base Amount */}
        <div style={{ width: 140, textAlign: "right", padding: "8px" }}>

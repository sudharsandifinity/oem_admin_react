...
import ViewGRPSideNavigation from "./ViewGRPSideNavigation"
...
const GRP = () => {
  ...
  const [formDetails, setFormDetails] = useState([]);
   const [originalGRPList, setOriginalGRPList] = useState([]);
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      try {
        let res = await dispatch(fetchPurchaseDeliveryNotes({top:pageSize,skip:0}))
        // if (formDetails[0]?.name === "GRP ") {
        //   res = await dispatch(
        ...
        const raw = res?.payload?.value ?? res?.payload ?? res;
        ...
        setOriginalGRPList(initialData);
        setPage(1);
      } catch (err) {
  ...
  const GRPTableCols =
    menuChildMap&&menuChildMap[0]?.menuName === "Purchase"
  ...
  const editRow = async (rowData) => {
    ...
    navigate("/GRP/edit/" + rowData.DocEntry);
  };
  const viewRow = async (rowData) => {
    console.log("rowData", rowData);
    navigate("/GRP/view/" + rowData.DocEntry);
  };
              ...
              {ManageSalesOderHeaderField.map((field) => {
                ...
                return (
                  <HeaderFilterBar
                    ...
                    originalGRPList={originalGRPList}
                    setOriginalGRPList={setOriginalGRPList}
                  />
                );
              })}
              {console.log("mclearfilter",customerorder)}
              <Button
               design="Default"
            ...
            <FlexibleColumnLayout
              ...
              startColumn={
                <FlexBox direction="Column">
                  <div>
                    <FlexBox direction="Column">{console.log("tableData",tableData)}
                      <AnalyticalTable
                        columns={columns}
                        data={tableData}
                        // header={`(GRP  - ${tableData.length})`}
                        rowStyle={(row) => {
                          const docLines = row.original?.DocumentLines || [];
                          const allQtyZero =
                            Array.isArray(docLines) &&
              ...
              midColumn={
                ...
                >
                  ...
                  >{console.log("viewItems",viewItem)}
                    <ViewGRPSideNavigation viewItem={viewItem} />

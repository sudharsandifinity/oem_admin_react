...
import VendorDialog from "./VendorDetails/VendorDialog";
...
const General = ({
  ...
  const handleVendorDialogOpen = () => setDialogOpen(true);
  const handleVendorDialogClose = () => setDialogOpen(false);

  useEffect(() => {
    console.log("formdetailgeneral", formDetails);
    const fetchData = async () => {
      try {
        let res = await dispatch(fetchPurBusinessPartner()).unwrap();
  ...
  useEffect(() => {
    ...
    if (autoVendorNameRef) {
      handleChange({
        target: { name: "CardName", value: autoVendorNameRef },
      });
    }
  }, [autoCustomerRef, autoContactPersonRef, autoVendorNameRef]);
            ...
            >
              ...
              <FlexBox direction="Column" style={{ width: "100%", gap: "8px" }}>
                <FlexBox alignItems="Center">
                  <Label style={{ minWidth: "200px" }}>Vendor:</Label>
                  <Controller
                    name="CardCode"
                    control={control}
                    render={({ field }) => (
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Input
                        ...
                        disabled={mode === "view"}
                        style={{ width: "100%" }}
                        value={autoVendorNameRef || field.value}
                        onInput={(e) => field.onChange(e.target.value)}
                        onChange={handleChange}
              ...
              <FlexBox direction="Column" style={{ width: "100%", gap: "8px" }}>
                ...
                <FlexBox alignItems="Center">
                  <Label style={{ minWidth: "200px" }}>Due Date</Label>
                  <Controller
                    name="DocDueDate"
                    control={control}
                    render={({ field }) => (
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      value={
                        selectedVendor
                          ? generalData.find(
                              (r) => r.Vendor === selectedVendor
                            )?.Vendor
                          : field.value
                      }
                      onInput={(e) => field.onChange(e.target.value)}
                      onChange={handleChange}
                      valueState={errors.Vendor ? "Error" : "None"}
                      icon={
                        <Button
